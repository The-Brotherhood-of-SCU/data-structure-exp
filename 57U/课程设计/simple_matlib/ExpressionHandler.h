...
public:
    ...
    double eval(const string& expression,const map<string,double>& variables={}){
        ...
        auto result= astRoot->eval(variables);
        AstNode::destroy(astRoot);
        return result;
    }
    AstNode* parse(const string& expression){
        auto infix=analyze(expression);
        auto postfix=infix2postfix(infix);
        return postfix2Ast(postfix);
    }
    ...
    void bracket_validate(const string& s){
        ...
        for(auto i :s){
            ...
            if(bracket_pair.find(i)!=bracket_pair.end()){
                ...
                if(mapped!=bracket_stack.top()){
                    throw Exception("invalid brackets: brackets not match)");
                } else{
                    bracket_stack.pop();
                }
            }
        ...
        if(!bracket_stack.empty()){
            throw Exception("invalid brackets: brackets not closed");
        }
    }
    ...
    AstNode* postfix2Ast(const vector<string>& postfix){
        ...
        for(const auto& i:postfix){
            if(isOperator(i)){
                if(isFunction(i)){
                    if(stack.size()<1){
                        throw Exception("function has no parameter: "+i);
                    }
                    ...
                    if(i=="!"){
                        if(stack.empty()){
                            throw Exception("parameter not enough: "+i);
                        }
                        auto ele=stack.top();
                        stack.pop();
                        op->left= ele;
        ...
        if(stack.size()==1){
            ...
            throw Exception("parameter is too many");
        }
    }
    ...
    static void prettyPrint(const vector<string>& list){
        cout<<'|';
    ...
    static void simplify(AstNode** root) {
    ...
    string toString(AstNode* node){
        if(node->type==ElementType::immediate){
            return Utils::to_string_with_precision(node->immediateNumber);
        } else if(node->type==ElementType::op){
            if(node->left!= nullptr&&node->right!= nullptr){//二元
                auto left=toString(node->left);
                auto right=toString(node->right);
                if (getPriority(node->op)>getPriority(node->left->op)) {
                    left="("+left+")";
                }
                if (getPriority(node->op)>=getPriority(node->right->op)) {
                    right="("+right+")";
                }
                return left+node->op+right;
            }else{
                return node->op+"("+toString(node->right)+")";

            }
        } else if(node->type==ElementType::function){
            return node->op+"("+toString(node->left)+")";
        }else if(node->type==ElementType::variable){
